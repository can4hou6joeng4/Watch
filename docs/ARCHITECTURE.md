...
直接写入路径仍须保留既有防护和桌面进程守卫。官方导入采用独立的目标目录保护，不再把全局进程名检查当作所有路径的共同要求；限定原生验证见兼容性矩阵。保留旧数据库迁移及已落盘的 `tui-chain`、schema 标识。
...
发布 build job 已依赖整个 `quality.yml`：根类型检查、隔离测试、前端构建，以及安装 Tauri 系统依赖后在 Linux 运行 Rust 桥接单测。P3 新增目标指纹变化后的零写入/链尾不推进、`verifyWrittenTurns` 重复 user 后缀匹配，以及 OpenCode 锁文件的 `dev`、`ino`、`birthtimeNs`、`size` 替换校验；Rust job 会先安装根 Node 依赖，以满足 Tauri resources 检查。本地 Node 226 项、Rust 7 项、类型检查、前端构建和差异格式检查通过，见 P3 本地证据；提交 `adce6a7` 对应的 GitHub Actions run `34374022914` 也通过，见 P3 远程证据。`npm test` 统一通过 `scripts/run-tests.mjs` 隔离 HOME 与数据库。单一 CI 宿主编译、跨平台类型检查和打包成功均不替代原生会话恢复验收。
