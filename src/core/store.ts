...
export class Store {
  ...
  private migrate(): void {
    ...
    while (version < SCHEMA_VERSION) {
      ...
      if (!step) throw new Error(`watch.db schema 版本 ${version} 无迁移路径`);
      step();
      version += 1;
    ...
    for (const t of REQUIRED_TABLES) {
      ...
      if (!found) throw new Error(`watch.db 缺少必需表: ${t}`);
    }
  }
