...
export { opencodeDbPath, opencodeSessionsRoot } from './paths.js';

const BASE62 = '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';

/**
 * opencode 原生 ID：`prefix_` + 12 位时间 hex + 14 位 Base62 随机。
 * 必须按原生格式生成：part/message 排序以 id 作同时间 tiebreaker，且 opencode 的
 * Identifier.timestamp() 按 hex 解析前 12 位——base36 等自造格式会让它抛 BigInt 异常。
 */
function sortableId(prefix: string, timeMs: number, seq: number, descending = false): string {
  const current = BigInt(Math.floor(timeMs)) * 0x1000n + BigInt(seq % 0x1000);
...
/** 按原生 session id 全局反查 OpenCode 会话：SQLite 读 session.directory 作 cwd；db 路径走 WATCH_OPENCODE_DB */
...
export async function createEmptyOpencodeSession(cwd: string): Promise<SessionRef> {
  ...
  const t = nowMs();
  const sessionId = sortableId('ses', t, 1, true);
