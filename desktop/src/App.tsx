...
export default function App() {
  ...
  // 目标被占用等写入拒绝：只对「同一来源 + 同一目标」展示，来源/目标一变自然失效
  ...
  const blockedHere = blockedNotice && blockedNotice.provider === target && blockedNotice.sourceId === id ? blockedNotice : null;
  ...
  async function handoff() {
    ...
    try {
      if (!completed) {
        ...
        else { const data = demo ? { ok: true, sessionId: DEMO_TARGET, cwd: demoSource.cwd, notes: ['演示转换：不代表该方向通过原生验收。'] } : await invoke<{ ok?: boolean; sessionId?: string; cwd?: string; notes?: string[]; error?: string; blocked?: boolean; code?: string; holders?: { pid: number; command: string }[]; hint?: string }>('open_in', { sourceId: id, providerId: target }); if (!data.ok || !data.sessionId) { const failure = new Error(data.error || '转换失败') as Error & { info?: Blocked }; if (data.blocked) failure.info = { code: data.code ?? 'blocked', sessionId: data.sessionId, holders: data.holders, hint: data.hint }; throw failure; } setBlockedNotice(null); completed = { provider: target, sessionId: data.sessionId, cwd: data.cwd, notes: data.notes ?? [] }; }
        setTransfer(completed);
      }
      await deliver(completed); void loadRecents();
    } catch (error) { const info = (error as { info?: Blocked }).info; if (info) setBlockedNotice({ provider: target, sourceId: id, info, error: message(error) }); if (completed) setTransfer({ ...completed, error: message(error) }); toast(message(error), 'error'); }
    ...
    <div className="app-content">
      ...
      {page === 'handoff' && <>
        ...
        <section className="target-section" aria-labelledby="target-title"><div className="section-heading"><h2 id="target-title">接力目标</h2><span>06 AGENTS</span></div><div className="agent-selector" role="radiogroup" aria-label="接力目标 Agent">{AGENTS.map((entry, index) => { const installed = providers.find((provider) => provider.id === entry.id); return <button role="radio" aria-checked={target === entry.id} tabIndex={target === entry.id ? 0 : -1} key={entry.id} className={`agent-option ${target === entry.id ? 'selected' : ''}`} disabled={busy !== null} onClick={() => setTarget(entry.id)} onKeyDown={(event) => {
          ...
          </div> : <div className="standard-handoff"><p className="route-description">{route === 'resume' ? '使用原会话 ID 和来源工作目录恢复。' : '转入目标后生成恢复命令。不同格式可能降级或丢失部分记录；具体说明以转换结果为准。'}</p>{!ready && <p className="muted">等待来源识别完成</p>}{command && <div className="command-output"><span>$</span><code>{command}</code><button className="icon-button" title="复制恢复命令" aria-label="复制恢复命令" onClick={() => copyFeedback(command)}><Copy size={16} /></button></div>}{blockedHere && <div className="transfer-result blocked-message" role="alert"><span className="blocked-heading"><Info size={16} />目标会话被占用，本次未写入任何内容</span>{blockedHere.info.sessionId && <div className="blocked-target"><span>目标会话</span><code>{blockedHere.info.sessionId}</code></div>}{!!blockedHere.info.holders?.length && <p className="warning-message">占用者：{blockedHere.info.holders.map((holder) => `PID ${holder.pid} ${holder.command}`).join('、')}</p>}<p className="warning-message">{blockedHere.error}</p>{blockedHere.info.hint && <p className="muted">{blockedHere.info.hint}</p>}</div>}{activeTransfer && <div className="transfer-result"><span className="success-message"><Check size={16} />{route === 'resume' ? '恢复命令已生成' : '目标会话已生成'}</span><code>{activeTransfer.sessionId}</code>{activeTransfer.notes.map((note, index) => <p className="warning-message" key={index}>{note}</p>)}{activeTransfer.error && <p className="warning-message" role="alert">{activeTransfer.error}。重试仅交付恢复命令，不重复转换。</p>}</div>}<div className="delivery-options"><label><Terminal size={15} />{modeSelect('交付方式')}</label><label className="toggle-label"><input type="checkbox" checked={yolo} disabled={busy !== null || !agent.yolo} onChange={(event) => setYolo(event.target.checked)} /><span>YOLO</span></label></div>{yolo && agent.yolo && <p className="warning-message"><Info size={15} />免确认参数会放宽目标 Agent 的权限或沙箱保护。</p>}<button className="primary-button" disabled={!ready || busy !== null} onClick={() => { if (blockedHere) setBlockedNotice(null); void handoff(); }}>{busy ? <LoaderCircle className="spin" size={16} /> : <ArrowRight size={16} />}{activeTransfer ? mode === 'copy' ? '再次复制命令' : '仅重新打开' : blockedHere ? '重试转入' : route === 'resume' ? mode === 'copy' ? '复制恢复命令' : '在终端中继续' : mode === 'copy' ? '转入并复制命令' : '转入并启动终端'}</button></div>}
          </div>
