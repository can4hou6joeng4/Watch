...
import { appendFileAtomic, digestFile, writeFileAtomic } from '../src/core/atomic.js';
import { addFinding, findingsStatus, mergeFindings } from '../src/core/findings.js';
import { assertRoundTrip, verifyWrittenTurns } from '../src/core/verify.js';
...
test('writeFileAtomic: verify 抛错零写盘，原文件不损坏、无 tmp 残留', async () => {
  const dir = await mkdtemp(path.join(tmpdir(), 'watch-atomic-'));
  try {
    ...
    assert.ok(!files.some((f) => f.includes('watch-tmp')), `tmp 残留: ${files.join(',')}`);
  } finally {
...
test('appendFileAtomic: verify 抛错零写盘，原文件不被追加且无 tmp 残留', async () => {
  const dir = await mkdtemp(path.join(tmpdir(), 'watch-atomic-append-'));
  try {
    const file = path.join(dir, 'session.jsonl');
    await writeFile(file, 'LINE1\n', 'utf8');
    await assert.rejects(
      appendFileAtomic(file, 'LINE2\n', {
        backup: true,
        verify: async () => {
    ...
    const files = await readdir(dir);
    assert.ok(!files.some((f) => f.includes('watch-tmp')), `tmp 残留: ${files.join(',')}`);
    assert.ok(!files.includes('session.jsonl.bak'), '失败时不应保留/生成 .bak');
  } finally {
    await rm(dir, { recursive: true, force: true });
  }
});

test('appendFileAtomic: 既有文件追加成功，backup 生成 .bak', async () => {
  const dir = await mkdtemp(path.join(tmpdir(), 'watch-atomic-append-'));
  try {
    const file = path.join(dir, 'session.jsonl');
    await writeFile(file, 'LINE1\n', 'utf8');
    await appendFileAtomic(file, 'LINE2\n', { backup: true, verify: async () => {} });
    assert.equal(await readFile(file, 'utf8'), 'LINE1\nLINE2\n');
    assert.equal(await readFile(`${file}.bak`, 'utf8'), 'LINE1\n');
    // 无 backup 时不覆盖上一代 .bak
    await appendFileAtomic(file, 'LINE3\n', { verify: async () => {} });
    assert.equal(await readFile(file, 'utf8'), 'LINE1\nLINE2\nLINE3\n');
    assert.equal(await readFile(`${file}.bak`, 'utf8'), 'LINE1\n');
  } finally {
    await rm(dir, { recursive: true, force: true });
...
test('appendFileAtomic: 目标文件不存在时直接以 0600 创建新文件', async () => {
  const dir = await mkdtemp(path.join(tmpdir(), 'watch-atomic-append-'));
  try {
    const file = path.join(dir, 'new.jsonl');
    await appendFileAtomic(file, 'INIT\n', { backup: true });
    assert.equal(await readFile(file, 'utf8'), 'INIT\n');
    const { stat } = await import('node:fs/promises');
    assert.equal((await stat(file)).mode & 0o777, 0o600);
    const files = await readdir(dir);
    assert.ok(!files.includes('new.jsonl.bak'), '新文件首次追加不产生 .bak');
  } finally {
    await rm(dir, { recursive: true, force: true });
  }
});
...
test('verifyWrittenTurns: 丢弃首个 user 前记录后可从重复 user 候选正确匹配后缀', async () => {
  const expected = [
    turn('assistant', '目标格式会丢弃的前导记录'),
    turn('user', '重复问题'),
    turn('assistant', '最新回答', [ev('tool_call', 'call-1'), ev('tool_result', 'call-1')]),
  ];
  const parsed = [
    turn('user', '重复问题'),
    turn('assistant', '旧回答'),
  ...
  await assert.doesNotReject(
    verifyWrittenTurns({
      parse: async () => parsed,
      filePath: '/isolated/session.jsonl',
      turns: expected,
      provider: 'test',
    }),
  );
...
test('verifyWrittenTurns: 支持仅含 assistant 记录的增量末尾对账', async () => {
  const parsed = [
    turn('user', '问题一'),
    turn('assistant', '回答一'),
    turn('assistant', '追加的探查回复', [ev('tool_call', 'call-2'), ev('tool_result', 'call-2')]),
  ];
  const incremental = [
    turn('assistant', '追加的探查回复', [ev('tool_call', 'call-2'), ev('tool_result', 'call-2')]),
  ];
