...
before(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'watch-opencode-'));
  testDb = join(tmpDir, 'opencode.db');
  process.env.WATCH_OPENCODE_DB = testDb;
});

after(() => {
  delete process.env.WATCH_OPENCODE_DB;
});
...
test('importTurnsToOpencode：全新空库自举（不依赖 pre-init schema）', async () => {
  ...
  const bareDir = mkdtempSync(join(tmpdir(), 'watch-opencode-bare-'));
  const bareDb = join(bareDir, 'opencode.db');
  const old = process.env.WATCH_OPENCODE_DB;
  process.env.WATCH_OPENCODE_DB = bareDb;
  try {
    const ref = await importTurnsToOpencode(
      [
    ...
    assert.match(ref.sessionId, /^ses_[0-9a-f]{12}[0-9A-Za-z]{14}$/);
    const nativeDb = new DatabaseSync(bareDb, { readOnly: true });
    const sessionRow = nativeDb.prepare('SELECT id, time_created FROM session WHERE id = ?').get(ref.sessionId) as { id: string; time_created: number };
    const firstMessage = nativeDb.prepare('SELECT id, time_created FROM message WHERE session_id = ? ORDER BY time_created, id LIMIT 1').get(ref.sessionId) as { id: string; time_created: number };
    nativeDb.close();
    assert.equal(
      BigInt(`0x${sessionRow.id.slice(4, 16)}`),
      (~(BigInt(sessionRow.time_created) * 0x1000n + 1n)) & 0xffffffffffffn,
    );
    assert.equal(
      BigInt(`0x${firstMessage.id.slice(4, 16)}`),
      (BigInt(firstMessage.time_created) * 0x1000n + 1n) & 0xffffffffffffn,
    );
    const back = await parseOpencodeSession(ref);
    assert.equal(back.length, 2);
    ...
    process.env.WATCH_OPENCODE_DB = old;
  }
});

test('parseOpencodeSession：空 filePath 仍使用隔离数据库路径', async () => {
  initSchema();
  const sessionId = 'ses_env_fallback';
  const cwd = '/tmp/opencode-env-fallback';
  seedSession(sessionId, cwd, 'env fallback', 6000);
  seedMessage(sessionId, 'msg_env_fallback', 'user', 6001);
  seedPart(sessionId, 'msg_env_fallback', { type: 'text', text: 'isolated' }, 6002);

  const turns = await parseOpencodeSession({ provider: 'opencode', sessionId, filePath: '', cwd });

  assert.equal(turns.length, 1);
  assert.equal(turns[0]!.text, 'isolated');
});
