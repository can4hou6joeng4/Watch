...
import { buildCdFor, buildCommandFor, openInProvider, resolveById } from '../src/open.js';
import type { ProviderAdapter } from '../src/providers/adapter.js';

const UUID = '461fa5e9-71f1-4bf1-972d-8a41b479905c';

function tmp(): string {
  const dir = mkdtempSync(join(tmpdir(), 'watch-open-'));
...
test('opencode findById: SQLite 按 id 查 session.directory', async () => {
  ...
  process.env.WATCH_OPENCODE_DB = dbPath;
  try {
    ...
    if (old === undefined) delete process.env.WATCH_OPENCODE_DB;
    else process.env.WATCH_OPENCODE_DB = old;
  }
});
...
test('openInProvider: 属主即目标时快路径返回，mergedTurns=0', async () => {
  const store = new Store(':memory:');
  const root = tmp();
  const file = join(root, 'test.txt');
  writeFileSync(file, 'test\n');
  store.rememberSessionLocation('pi', { provider: 'pi', sessionId: 'ses-pi-1', filePath: file, cwd: '/test/cwd' });
  const res = await openInProvider('ses-pi-1', 'pi', store);
  assert.equal(res.ok, true);
  assert.equal(res.provider, 'pi');
  assert.equal(res.mergedTurns, 0);
  assert.equal(res.cwd, '/test/cwd');
});