...
import { chainStatus, chainUsage, diffTurns, handoff, HandoffBlockedError, pairedRef } from '../src/core/handoff.js';
...
test('findLatestClaudeSession: mtime 最新 uuid 胜出，非法名忽略，空目录 null', async () => {
  const root = mkdtempSync(join(tmpdir(), 'watch-claude-'));
  const cwd = '/proj/demo';
...
test('findLatestCodexRollout: 其它 cwd 的更新文件不得胜出', async () => {
  const root = mkdtempSync(join(tmpdir(), 'watch-codex-'));
  const cwd = '/proj/demo';
...
test('findLatestCodexRollout: 只读首行 cwd，旧日期目录仍能命中', async () => {
  const root = mkdtempSync(join(tmpdir(), 'watch-codex-old-'));
  const cwd = '/proj/demo';
...
test('handoff: 目标指纹在解析后变化时阻断写入且不推进链尾', async () => {
  const store = new Store(':memory:');
  const cwd = '/project with spaces';
  const claudeRef: SessionRef = { provider: 'claude', sessionId: 'c1', filePath: '/tmp/c1.jsonl', cwd };
  const codexRef: SessionRef = { provider: 'codex', sessionId: 'x1', filePath: '/tmp/x1.jsonl', cwd };
  const sourceTurns: UnifiedTurn[] = [
    { role: 'user', text: 'u1', timestamp: '2026-09-09T00:00:00.000Z' },
    { role: 'assistant', text: 'a1', timestamp: '2026-09-09T00:00:01.000Z', provider: 'claude' },
    { role: 'user', text: 'u2', timestamp: '2026-09-09T00:00:02.000Z' },
  ];
  const destTurns = sourceTurns.slice(0, 2);
  let fingerprintReads = 0;
  let importCalls = 0;

  const adapters: ProviderAdapter[] = [
    {
      ...stubAdapter('claude', { ref: claudeRef, updatedAt: 200 }, sourceTurns),
    },
    {
      ...stubAdapter('codex', { ref: codexRef, updatedAt: 100 }, destTurns),
      async contentFingerprint() {
        fingerprintReads += 1;
        return fingerprintReads === 1
          ? { sha256: 'a'.repeat(64), sizeBytes: 10 }
          : { sha256: 'b'.repeat(64), sizeBytes: 11 };
      },
      async importTurns(_turns, _dir, into) {
        importCalls += 1;
        return into ?? codexRef;
      },
    },
  ];
  const chainId = store.getOrCreateChainSession(cwd);
  store.recordSwitch(chainId, 'claude', claudeRef, 'codex', codexRef);
  store.recordSwitch(chainId, 'codex', codexRef, 'claude', claudeRef);
  const before = store.listSwitchEvents(chainId);

  await assert.rejects(
    handoff('codex', cwd, adapters, (id) => adapters.find((adapter) => adapter.id === id), store),
    (error: unknown) => {
      assert.ok(error instanceof HandoffBlockedError);
  ...
  assert.deepEqual(store.listSwitchEvents(chainId), before);
});

test('cli --json status 形状可用（子进程）', async () => {
  const { spawnSync } = await import('node:child_process');
  // 库走临时 WATCH_DB、cwd 用临时目录：不碰真实 ~/.watch（否则会触发 migrateLegacyDb），也不依赖本机真实会话
  const dir = mkdtempSync(join(tmpdir(), 'watch-cli-status-'));
  try {
    const r = spawnSync(
      'npx',
      ['tsx', 'src/cli.ts', 'status', '--json', '--cwd', dir],
      { encoding: 'utf8', cwd: process.cwd(), env: { ...process.env, WATCH_DB: join(dir, 't.db') } },
    );
    const line = (r.stdout || '')
      .split('\n')
      .map((l) => l.trim())
...
test('cli chain new/list --json 走独立 WATCH_DB', async () => {
  const { spawnSync } = await import('node:child_process');
  const dir = mkdtempSync(join(tmpdir(), 'watch-cli-chain-'));
  const db = join(dir, 't.db');
  const env = { ...process.env, WATCH_DB: db };
  try {
