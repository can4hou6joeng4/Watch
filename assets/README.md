...
| 文件 | 内容 | 生成方式 | 边界 |
|---|---|---|---|
| `cli-claude-to-opencode.gif` | CLI 链路径：`status` → `sessions` → `switch opencode` → `status` | 在临时 `HOME` / `WATCH_DB` / `WATCH_OPENCODE_DB` 下真实运行 CLI，输出与耗时录入 `cli-transcript.json`，再由 `replay-cli-gif.mjs` 在无头 Chrome 中按真实时序回放并合成 GIF。画布与桌面截图一致（1280×1120），终端配色取 `desktop/src/App.css` 的亮色令牌 | 来源是 `make-demo-fixture.mts` 生成的合成 Claude 会话；写入的是临时 OpenCode 库；路径已改写为 `~/…` 便于阅读；未启动任何原生客户端，不代表 OpenCode 原生续写通过 |
...
# 2. 录制 CLI 输出（会重置临时库）。run-demo-cli.sh 只在终端里带色回放；
#    四条命令的 stdout 与耗时整理为 assets/cli-transcript.json（仓库跟踪），replay 脚本读它
bash scripts/media/run-demo-cli.sh
...
# 5. GIF 回放合成（需要 ffmpeg 与本机 Chrome）；默认读 assets/cli-transcript.json，
#    可用 TRANSCRIPT=… OUT=… 覆盖；输出画布 1280×1120，与桌面截图同比例
